...
Este capítulo presenta un resumen de los logros y las dificultades para llegar al estado actual del tabajo y sugiere una serie de siguientes pasos para la evolución del sistema.
...
el trabajo está en producción, y clasifica las canciones que ingresan al sistema. En relación a los objetivos se puede concluir que:
...
\begin{itemize}
\item Se cumplió la totalidad de los requerimientos técnicos al lograr la caracterización de las variables solicitadas a partir de los datos de plusyc.
...
\item La decisión de usar PyTorch y PANNs permitió no depender de licencias ni pagar costos por consultas a modelos de terceros, también facilitó el trabajo al no requerir de arquitecturas complejas.
...
\item Para la detección de público en vivo y habla en canciones, se requirió de limpieza de ruido por falsas etiquetas apoyada en modelos preentrenados.
...
Algunos aspectos que se podrían mejorar o explorar en el futuro son:

\begin{itemize}
\item Los modelos actuales quedan como base para iteraciones futuras.
\item Se deja como pendiente usar las inferencias de PANNs para mejorar la limpieza de algunos datos y corregir audios mal etiquetados.
\item La predicción de instrumentalness es un caso que se sugiere revisar puntualmente, ya que presenta ruido en los valores intermedios por falta de definiciones claras.
El modelo actual logra diferenciar una canción instrumental de una que no lo es, pero los valores en el medio no son claros.
\item También se sugiere iterar sobre el tempo para buscar una solución más robusta que el algoritmo de DSP.
...
\item El sistema se puede ajustar para el despliegue bajo el modelo de software como servicio para ofrecer el servicio al público interesado.
\item Se podría evaluar a futuro el uso de arquitecturas basadas en transformadores para audios, si se cuenta con mayor capacidad de cómputo, para buscar mejoras en la clasificación de clases densas.
\end{itemize}

% TODO REVISAR LO DE TRANSFORMERS Y OTRAS TÉCNICAS MÁS RECIENTES Y PESADAS.
%----------------------------------------------------------------------------------------

...
El análisis comparativo de las fuentes de datos, las representaciones estadísticas de las canciones y los criterios de selección de variables predictivas se detallan en la sección \ref{sec:conjuntos_datos} del siguiente capítulo.

\section{Representaciones numéricas de la música}
\label{sec:dsp}

Una canción se traduce a un formato numérico para su procesamiento computacional. El procesamiento digital de señales (DSP) transforma la onda sonora en arreglos matemáticos de una o dos dimensiones \cite{muller2015fundamentals, lerch2012introduction}. 
Entre las representaciones bidimensionales más destacadas se encuentran el espectrograma, el cromagrama y el espacio armónico o tonnetz. El espectrograma refleja la variación de las frecuencias a lo largo del tiempo \citep{muller2015fundamentals}. Por su parte, el cromagrama captura la energía asociada a las doce clases de notas musicales \citep{gomez2006tonal}. Finalmente, el tonnetz modela las relaciones tonales y armónicas de la pista \citep{harte2006detecting}. A modo de ejemplo, en la figura \ref{fig:representaciones_2d} se ilustran estas tres transformaciones aplicadas a un fragmento de la canción ``Weird Fishes/Arpeggi'' de la banda Radiohead. En estos gráficos, el eje horizontal representa el tiempo, mientras que la intensidad del color indica la magnitud de la energía para las distintas frecuencias, notas musicales o relaciones tonales correspondientes a cada eje vertical.
...
El listado completo de las variables de entrada, el análisis de su relevancia y su impacto en el rendimiento de los algoritmos se detalla en la sección \ref{sec:seleccion_variables}.
...
\begin{table}[htpb]
    ...
    \begin{tabular}{p{3.5cm} p{8.5cm}}    
        ...
        \midrule
        MFCC & Coeficientes cepstrales que modelan la percepción auditiva humana \citep{muller2015fundamentals}. \\       
        Centroide espectral & Indica el centro de masa del espectro acústico. Se asocia al brillo del sonido \citep{lerch2012introduction}. \\
        Ancho de banda & Mide la dispersión de las frecuencias alrededor del centroide espectral \citep{lerch2012introduction}. \\
        Tasa de cruces por cero & Cuantifica la cantidad de cambios de signo en la señal temporal \citep{lerch2012introduction}. \\
        Rolloff espectral & Estima la frecuencia por debajo de la que se concentra la mayor parte de la energía \citep{lerch2012introduction}. \\
        \bottomrule
    \end{tabular}
...
Para este fin existen bibliotecas especializadas, como PANNs Inference \citep{kong2020panns}. Esta herramienta provee acceso a redes neuronales optimizadas para el análisis de eventos sonoros. Dentro de sus opciones, destaca la arquitectura denominada CNN14, cuyo diseño se inspira en los modelos convolucionales profundos de la familia VGG \citep{simonyan2014very}. Se trata de una red neuronal capaz de abstraer patrones del audio y generar \textit{embeddings} numéricos basados en las clases del conjunto de datos AudioSet.
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=1\textwidth,height=0.45\textheight]{mapa_top_clases.pdf}
...
\begin{itemize}
    \item Algoritmos LightGBM: se integró la arquitectura LightGBM, basada en el ensamble de árboles de decisión. Esta herramienta destaca por su alta velocidad computacional y eficiencia en tareas generales de predicción \citep{ke2017lightgbm}.
    En este caso también se usó Optuna para la búsqueda de parámetros optimos \citep{akiba2019optuna}.
    \item Redes MLP: los perceptrones multicapa son redes neuronales artificiales con capacidad para modelar relaciones complejas. Permiten, entre otras aplicaciones, predecir múltiples variables continuas de forma simultánea \citep{goodfellow2016deep}.
\end{itemize}
...
El servicio fue contenedorizado con Docker \citep{merkel2014docker} y es llamado de manera asíncrona, desde una herramienta interna de Plusyc denominada Beethoven.
El alojamiento remoto del producto se apoyó en los servicios de infraestructura de OCI (por su sigla en inglés, \textit{Oracle Cloud Infrastructure}) \citep{oracle_oci}. Se provisionó una instancia de cómputo virtual en la nube para desplegar la aplicación y ejecutar los modelos predictivos. El uso de esta tecnología garantiza la disponibilidad continua del sistema.
